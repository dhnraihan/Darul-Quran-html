let formSteps = document.querySelectorAll(".step-forms");
...
let parentsNameField = document.getElementById('parentsName');
let studentAgeField = document.getElementById('studentAge');
...
firstStepBtn.addEventListener('click', e => {
    ...
    if (!phone) {
        ...
        phone = landingIntTel.getNumber(); //landingIntTel defined on landing blade.

        if (!isPhoneValid(phone)) {
            toastr.error("Please Insert A valid Phone Number!");
            phoneField.focus();
        } else {
            formLoading(form);
            axios.post(`/trialRequestFirst`, { phone, name })
                .then(res => {
...
secondStepBtn.addEventListener('click', e => {
    ...
    let parentsName = parentsNameField.value;
    let studentAge = studentAgeField.value;
    ...
    axios.post(`/trialRequestSecond`, {id, parentsName, email, studentAge})
        .then(res => {
            let { status, message } = res.data;

            if (status) {
                clearForm(form);
                landingIntTel.setCountry("us");
                formStepsNum--;
                updateFormSteps();
                formLoadingStop(form, 'Schedule Your Free Class');
